...
namespace Rabin_Cryptosystem_Application
{
    public partial class MainForm : Form
    {
        ...
        private void btPrimeSave_Click(object sender, EventArgs e)
        {
            ...
            savePrimeFile.Filter = "Rabin Prime Number File (*.RabinPrime)|*.RabinPrime";
            savePrimeFile.Title = "Save A Rabin Prime Number File As";
            ...
            if (savePrimeFile.FileName != "")
            {
                try
                {
                    if (!savePrimeFile.FileName.EndsWith(".RabinPrime"))
                        savePrimeFile.FileName += ".RabinPrime";
        ...
        /// <summary>
        /// Generates a random prime number of an appropriate size.
        /// </summary>
        private void PrimeNumberGeneration()
        {
            ...
            for (; ; number += two)
            {
                ...
                /* we eliminate the case in which the Tonneli-Shanks algorithm
                 * does a lot more computations than in the cases p = 3,5,7
                 * (mod 8) */
                if (number % eight == one)
                {
                    probablyPrime = false;
                    ...
                    guiChange = delegate()
                    {
                        tbPrimeGenLog.Text += "Disregarding current number, as it is equal to 1 modulo 8 " +
                                              "(slow computations).\r\n";
                    };
                    this.Invoke(guiChange);
                }
