...
public class PEM
{
    ...
    /// <summary>
    ///     Construct a PEM from the public key and private key in RSA, if convertToPublic RSA with private key will only read
    ///     the public key, RSA with only public key will not be affected
    /// </summary>
    ...
    /// <summary>
    ///     Construct a PEM from the full amount of PEM field data. Except for modulus and public key exponent, all other
    ///     private key exponent information must be provided or not (the exported PEM contains only the public key)
    ...
    /// <summary>
    ///     Construct a PEM from the public key index and the private key index, and calculate P and Q in reverse, but they are
    ///     very unlikely to be the same as the P and Q of the original generated key
    ///     Note: If the first byte of all parameters is 0, you must remove it first
    ...
    /// <summary>
    ///     Convert the public key private key in the PEM into an RSA object. If the private key is not provided, only the
    ///     public key will be included in the RSA
    /// </summary>
    ...
    /// <summary>
    ///     Convert to a positive integer, if it is a negative number, you need to add a leading 0 to convert to a positive
    ///     integer
    /// </summary>
    ...
    /// <summary>
    ///     The first byte of BigInt exported byte integer> 0x7F will add a leading 0 to ensure a positive integer, so 0 needs
    ///     to be removed
    /// </summary>
    public static byte[] BigB(BigInteger bigx)
    {
        var val = bigx.ToByteArray().Reverse().ToArray(); // C#'s binary is reversed
        if (val[0] == 0)
        {
    ...
    /// <summary>
    ///     Some key parameters may be less than one bit (32 bytes only have 31. Visual inspection is a problem with the key
    ///     generator. This parameter is only found in the key generated by c#, but the key generated in java does not This
    ///     phenomenon), just correct it directly; this problem is fundamentally different from BigB, and BigB cannot be
    ///     touched.
    /// </summary>
    ...
    private static BigInteger FindFactor(BigInteger e, BigInteger d, BigInteger n)
    {
        ...
        var s = -1;
        if (edMinus1 != BigInteger.Zero) s = (int)(BigInteger.Log(edMinus1 & -edMinus1) / BigInteger.Log(2));
        var t = edMinus1 >> s;
        ...
        for (var aInt = 2;; aInt++)
        {
            if (aInt % 10 == 0 && DateTime.Now.Ticks - now > 3000 * 10000)
                throw
                    new Exception(
                        "estimate RSA.P timeout"); //The test loops up to 2 times, and the 1024-bit speed is very fast 8ms

            var aPow = BigInteger.ModPow(new BigInteger(aInt), t, n);
            for (var i = 1; i <= s; i++)
            {
                if (aPow == BigInteger.One) break;
                if (aPow == n - BigInteger.One) break;
                var aPowSquared = aPow * aPow % n;
                if (aPowSquared == BigInteger.One) return BigInteger.GreatestCommonDivisor(aPow - BigInteger.One, n);
                aPow = aPowSquared;
            }
        }
    }
    ...
    ///     Error will throw an exception
    /// </summary>
    public static PEM FromPEM(string pem)
    {
        var param = new PEM();
        ...
        if (data == null) throw new Exception("PEM content is invalid");
        var idx = 0;
        ...
        var readBlock = () =>
        {
            ...
            for (var i = 0; i < len; i++) val[i] = data[idx + i];
            idx += len;
            return val;
        };
        ...
        else if (pem.Contains("PRIVATE KEY"))
        {
            ...
            if (!eq(_Ver)) throw new Exception("PEM unknown version");
            ...
            if (eq(_SeqOID))
            {
                ...
                //Read the version number
                if (!eq(_Ver)) throw new Exception("PEM version is invalid");
            }
            else
            {
                idx = idx2;
    ...
    ///     convertToPublic: When equal to true, the RSA with the private key will only return the public key, and the RSA with
    ///     only the public key will not be affected
    ///     Public key such as: -----BEGIN RSA PUBLIC KEY-----, private key such as: -----BEGIN RSA PRIVATE KEY-----
    ///     It seems that the public key of PKCS#1 is less used to export, the public key of PKCS#8 is used more, and the
    ///     private key #1#8 is almost the same.
    /// </summary>
    ...
    ///     convertToPublic: When equal to true, the RSA with the private key will only return the public key, and the RSA with
    ///     only the public key will not be affected
    ///     privateUsePKCS8: The return format of the private key. When it is true, it will return PKCS#8 format (-----BEGIN
    ///     PRIVATE KEY-----), otherwise it will return PKCS#1 format (-----BEGIN RSA PRIVATE KEY-- ---), this parameter is
    ///     invalid when returning the public key; both formats are more common
    ///     publicUsePKCS8: The return format of the public key. When it is true, it returns PKCS#8 format (-----BEGIN PUBLIC
    ///     KEY-----), otherwise it returns PKCS#1 format (-----BEGIN RSA PUBLIC KEY-- ---), this parameter is invalid when
    ///     returning the private key; generally, the true PKCS#8 format public key is used, and the PKCS#1 format seems to be
    ///     relatively rare.
    /// </summary>
    public string ToPEM(bool convertToPublic, bool privateUsePKCS8, bool publicUsePKCS8)
    {
        ...
        void writeAll(MemoryStream stream, byte[] byts) { stream.Write(byts, 0, byts.Length); }
        ...
        string TextBreak(string text, int line)
        {
            ...
            while (idx < len)
            {
                ...
                if (idx + line >= len)
                    ...
                    _ = str.Append(text.AsSpan(idx, line));
                idx += line;
            }
        ...
        if (Key_D == null || convertToPublic)
        {
            ...
            var flag = "PUBLIC KEY";
            if (!publicUsePKCS8) flag = "RSA" + flag;
        ...
        else
        {
            ...
            if (!privateUsePKCS8) flag = " RSA" + flag;
            return "-----BEGIN" + flag + "-----\n" + TextBreak(Convert.ToBase64String(byts), 64) + "\n-----END" + flag +
                   "-----";
        }
    }
    ...
    ///     Error will throw an exception
    /// </summary>
    public static PEM FromXML(string xml)
    {
        var rtv = new PEM();

        var xmlM = xmlExp.Match(xml);
        if (!xmlM.Success) throw new Exception("XML content does not meet the requirements");
        ...
        if (rtv.Key_Modulus == null || rtv.Key_Exponent == null) throw new Exception("XML public key is lost");
    ...
    ///     , if convertToPublic RSA with private key will only return public key, RSA with only public key will not be
    ///     affected
    /// </summary>
