...
namespace CryptoUtility
{
        ...
        where T : unmanaged
    {
        ...
        {
            public CudaProgressMemoryBuffer(CudaAccelerator accelerator, long length, int elementSize) : base(accelerator, length, elementSize)
            {
                CudaException.ThrowIfFailed(CudaAPI.CurrentAPI.AllocateHostMemory(out IntPtr resultPtr, new IntPtr(LengthInBytes)));
                NativePtr = resultPtr;
            }
            
            protected override void DisposeAcceleratorObject(bool disposing)
            {
                CudaException.ThrowIfFailed(CudaAPI.CurrentAPI.FreeHostMemory(NativePtr));
